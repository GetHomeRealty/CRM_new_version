...
const editLine = { border: '1px solid #e6e8ef', borderRadius: 4, padding: '2px 6px', fontSize: 13, minWidth: 180 };

// Listing types where the Commission Received Date is not shown in the Agent FAQ Center.
const HIDE_COMM_DATE_TYPES = [
...
export default function AgentFaqModal({ open, onClose, transactionId, txn, onSaved, termCount: termCountProp, depositSlipOnly = false, dftNA = false, readOnly = false, allowBatchEmail = false, isAgent = false }) {
  ...
  const invAdmin = txn.invoice_admin || {}; // auto invoice fields (mirrors Admin Activities)
  ...
  const [form, setForm] = useState(() => {
    ...
    return {
      ...
      deposit_slips: Array.isArray(a.deposit_slips) ? a.deposit_slips : [],
      void_cheque: a.void_cheque || null,
      // Client Payment breakdown — editable fields.
      client_name: a.client_name || (txn.clients || []).map((c) => c.name).filter(Boolean).join(', '),
      void_cheque_name: a.void_cheque_name || '',
      client_inst_no: a.client_inst_no || '',
      client_transit_no: a.client_transit_no || '',
      client_account_no: a.client_account_no || '',
    };
  });
  ...
  const addSlips = (files) => {
    Array.from(files || []).forEach((file) => {
      const reader = new FileReader();
      reader.onload = () => setForm((f) => ({ ...f, deposit_slips: [...(f.deposit_slips || []), { name: file.name, data: reader.result }] }));
      reader.readAsDataURL(file);
    });
  };
  const rmSlip = (i) => setForm((f) => ({ ...f, deposit_slips: (f.deposit_slips || []).filter((_, idx) => idx !== i) }));
  const setVoidCheque = (file) => {
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => setForm((f) => ({ ...f, void_cheque: { name: file.name, data: reader.result } }));
    reader.readAsDataURL(file);
  };
  ...
  const dealComm = listing ? (fin.listing || {}) : { commission: fin.commission, hst: fin.hst, total: fin.total };
  ...
  const signedAdj = (v) => (v === 0 ? money(0) : `${v > 0 ? '-' : '+'}${money(Math.abs(v))}`);
  const bRow = (label, val, strong) => `<div style="display:flex;justify-content:space-between;padding:9px 14px;border-bottom:1px solid #f1f5f9;font-size:13px"><span style="color:#334155">${label}</span><span style="${strong ? 'font-weight:600;' : ''}">${val}</span></div>`;
  const bBar = (label) => `<div style="background:#eef2f7;padding:8px 14px;text-align:right;font-size:11px;font-weight:700;color:#64748b;letter-spacing:.04em;text-transform:uppercase">${label}</div>`;
  const agentBlockHtml = (a) => {
    ...
    return `
      <div style="border:1px solid #e6e8ef;border-radius:10px;overflow:hidden;margin-bottom:14px">
        <div style="background:#f8fafc;padding:8px 14px;font-size:11px;font-weight:700;letter-spacing:.04em;color:#475569;text-transform:uppercase">Agent Name</div>
  ...
  const agentDocHtml = (fin.agents || []).length
    ? (fin.agents || []).map(agentBlockHtml).join('')
  ...
  const pctAmt = (sec) => `${sec?.pct ? sec.pct + '% / ' : ''}${money(sec?.commission)}`;
  ...
  const ln = (val) => (val !== undefined && val !== null && String(val) !== '' ? String(val) : blank);
  const li = (label, val) => `<li style="margin-bottom:6px"><strong>${label}:</strong> ${val}</li>`;
  const clientDocHtml = `
    ...
    <div className="overlay open" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal xl">
        ...
        {isFaqV2 && (<>
          ...
          {form.agent_commission_paid_status === 'Yes' && (
            <div className="card" style={{ marginTop: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, gap: 8, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 12 }}>
          ...
          <div className="field" style={{ marginBottom: 0 }}><label style={lbl}>Commission Received Date</label><input type="date" value={form.commission_received_date} onChange={(e) => set('commission_received_date', e.target.value)} /><span className="help">Managed from Admin Activities.</span></div>
