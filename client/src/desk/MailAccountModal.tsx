...
import {
  ...
  type AccountMailAccount, type MailAccountInput, type IntegrationScope,
} from '../lib/accountApi';
 ...
 */
export default function MailAccountModal({ account, onClose, onSaved, scope }: {
  account: AccountMailAccount | null;
  onClose: () => void;
  onSaved: () => void;
  /** Which area is adding this account — it is stamped on creation so the two stay separate. */
  ...
  return createPortal(
    <div className="overlay open" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal">
        ...
        {step === 'choose' && (
          <ChooseProvider email={email} setEmail={setEmail} scope={scope}
            onGmail={() => setStep('gmail')} onSmtp={() => setStep('smtp')} onCancel={onClose} />
        )}
        {step === 'gmail' && (
          <ProviderForm mode="gmail" email={email} onBack={() => setStep('choose')} onClose={onClose} onSaved={onSaved} scope={scope} />
        )}
        {step === 'smtp' && (
          <ProviderForm mode="smtp" email={email} onBack={() => setStep('choose')} onClose={onClose} onSaved={onSaved} scope={scope} />
        )}
        {step === 'edit' && account && (
...
function ChooseProvider({ email, setEmail, onGmail, onSmtp, onCancel, scope }: {
  email: string; setEmail: (v: string) => void; onGmail: () => void; onSmtp: () => void; onCancel: () => void;
  /** The area adding this account, so the OAuth round trip lands in the right one. */
  scope?: IntegrationScope;
}) {
  ...
  const connectGoogle = async () => {
    setConnecting(true);
    try {
      const res = await mailGoogleConnect(scope);
      if (res.url) {
...
function ProviderForm({ mode, email, onBack, onClose, onSaved, scope }: {
  mode: 'gmail' | 'smtp'; email: string; onBack: () => void; onClose: () => void; onSaved: () => void;
  /** The area adding this account — stamped on the new record so the two stay separate. */
  scope?: IntegrationScope;
}) {
  const toast = useToast();
