              ...
              {!loading && leads.map((l) => (
                <tr key={l.id}>
                  ...
                  <td className="lead-actions">
                    <button className="btn ghost sm" type="button" onClick={() => navigate(`/app/lead/${l.id}`)}>View</button>
                    {canEdit && <button className="btn ghost sm" type="button" onClick={() => { setEditing(l); setEditorOpen(true); }}>Edit</button>}
                    {canEdit && (
                      <button className="btn ghost sm" type="button" onClick={() => void toggleClosed(l)}>
                        {l.lead_status === 'closed' ? 'Reopen' : 'Close'}
                      </button>
                    )}
                    {canEdit && !isBrokerageLead(l) && <button className="btn ghost sm" type="button" onClick={() => doDelete(l)}>Delete</button>}
                  </td>
                </tr>
              ))}
            </tbody>
