...
/** Structured properties returned by the Photon (komoot) geocoder. */
...
export default function AddTransactionModal({ open, onClose, onCreated }: AddTransactionModalProps) {
  ...
  const [addrSug, setAddrSug] = useState<PhotonFeature[]>([]);
  ...
  const rank = (p: PhotonProps = {}) => (p.housenumber ? 3 : 0) + (p.street ? 2 : 0) + (p.postcode ? 1 : 0);
  ...
  const fmtAddress = (s: PhotonFeature): string => {
  ...
  const pickAddr = (s: PhotonFeature) => {
  ...
  const addMember = (name: string) => { if (name && !teamMembers.includes(name)) setTeamMembers((m) => [...m, name]); };
  const removeMember = (name: string) => setTeamMembers((m) => m.filter((n) => n !== name));
  ...
  const submit = async () => {
    ...
    try {
      ...
      const msg = apiErrorMessage(err, 'Could not save transaction');
      setError(msg);
      toast(msg, 'bad');
    } finally {
      setSaving(false);
          ...
          <>
            ...
            {listing ? (
              ...
              <>
                ...
                <div className="g3">
                  <div className="field"><label>{isLease ? 'Total lease price' : /sale listing/i.test(form.type) ? 'Total Sale Price' : 'Total Purchase Price'} <span className="req">*</span></label>
