...
import {
  ...
  syncMailAccount, type AccountMailAccount,
} from '../lib/accountApi';
...
export default function EmailIntegrationCard() {
  ...
  const load = useCallback(() => { listMyMailAccounts().then(setAccounts).catch(() => setAccounts([])); }, []);
  useEffect(() => { load(); }, [load]);
  ...
  useEffect(() => {
    ...
    if (p.get('mail_connected') || p.get('mail_error')) {
      window.history.replaceState({}, '', '/app/email-settings?tab=crm');
      load();
    }
  }, [toast, load]);
