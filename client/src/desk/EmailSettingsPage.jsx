...
export default function EmailSettingsPage() {
  ...
  return (
    <>
      ...
      {tab === 'accounts' ? <AccountsTab toast={toast} /> : <TemplatesTab toast={toast} />}
    </>
  );
}
...
function AccountsTab({ toast }) {
  ...
  const onTest = async (a) => {
    ...
    catch (e) { toast(e.response?.data?.message || 'Test failed', 'bad'); }
  };
...
function AccountModal({ account, onClose, onSaved, toast }) {
  const isEdit = !!account.id;
  const [form, setForm] = useState({ ...EMPTY_ACCOUNT, ...account, password: '' });
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (isEdit) await updateMailAccount(account.id, payload); else await createMailAccount(payload);
      toast(isEdit ? 'Account updated' : 'Account created', 'ok');
      onSaved();
    } catch (e) { toast(e.response?.data?.message || 'Could not save', 'bad'); }
...
function TemplatesTab({ toast }) {
  ...
  const load = () => getEmailTemplates()
    .then((d) => { setGroups(d.groups || []); setAccounts(d.mail_accounts?.data ?? d.mail_accounts ?? []); })
...
function TemplateEditor({ template, accounts, onClose, onSaved, toast }) {
  const [form, setForm] = useState({
    subject: template.subject, body_html: template.body_html,
    mail_account_id: template.mail_account_id || '', is_active: template.is_active,
  });
  const [saving, setSaving] = useState(false);
