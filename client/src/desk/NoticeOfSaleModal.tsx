...
const ordinal = (i: number) => (i === 0 ? 'Salesperson' : `${ORD[i] || `${i + 1}th`} Salesperson`);
...
const card: CSSProperties = { border: '1px solid #e6e8ef', borderRadius: 14, padding: 20, marginBottom: 16 };
const label: CSSProperties = { fontSize: 11.5, color: '#334155', fontWeight: 600, marginBottom: 6, display: 'block' };
const boxField: CSSProperties = { background: '#f9fafb', border: '1px solid #e6e8ef', borderRadius: 8, padding: '9px 11px', fontSize: 13, color: '#0f172a', minHeight: 38 };
const boxInput: CSSProperties = { ...boxField, width: '100%', display: 'block' };
const sectionTitle: CSSProperties = { fontSize: 13, fontWeight: 700, letterSpacing: '.04em', textTransform: 'uppercase', margin: '4px 0 12px' };
const grid3: CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 12 };
const divider: CSSProperties = { borderTop: '1px solid #eef0f5', margin: '6px 0 16px' };

function Field({ label: lbl, value }: { label: ReactNode; value: ReactNode }) {
  return (
    <div>
...
interface NoticeOfSaleModalProps {
  open: boolean;
  onClose: () => void;
  txn: Transaction;
  onSaved?: (() => void) | null;
}
type LawyerField = 'name' | 'phone' | 'email';

export default function NoticeOfSaleModal({ open, onClose, txn, onSaved = null }: NoticeOfSaleModalProps) {
  const toast = useToast();
  const ref = useRef<HTMLDivElement>(null);
  const [docs, setDocs] = useState<DeskDocument[]>([]);

  const clients = txn.clients || [];
  const team: { name?: string | null; split?: number | string | null }[] = (txn.team && txn.team.length) ? txn.team : (txn.agent ? [{ name: txn.agent, split: 100 }] : []);
  ...
  const lw = (field: LawyerField): string => {
    const wantPrefix = useBuyerLawyer ? 'buyer_lawyer_' as const : 'seller_lawyer_' as const;
    const otherName = useBuyerLawyer ? txn.seller_lawyer_name : txn.buyer_lawyer_name;
    const own = txn[`${wantPrefix}${field}`];
    if (own) return own;
    // Legacy lawyer_* is safe only when the other side has no lawyer (so it can't be theirs).
    return otherName ? '' : (txn[`lawyer_${field}`] || '');
  };

  // Signatories = the deal's primary agent + any split agents (the Team), who sign at the bottom.
  const signatories = team.map((m) => m.name).filter((n): n is string => !!n);
  ...
  // Lease deals reframe the parties: Buyers → Tenants, Sellers → Landlords.
  const isLease = /lease/i.test(txn.type);
  const buyersLabel = isLease ? 'Tenants (Client Names)' : 'Buyers (Client Names)';
  const sellersLabel = isLease ? 'Landlords' : 'Sellers';
  const priceLabel = isLease ? 'Leased Price' : 'Purchase Price';
  const buyersPlaceholder = isLease ? 'Enter tenant name(s), comma separated' : 'Enter buyer name(s), comma separated';
  const sellersPlaceholder = isLease ? 'Enter landlord name(s), comma separated' : 'Enter seller name(s), comma separated';
  ...
  const [agents, setAgents] = useState<Record<string, AgentSignature>>({}); // name => { signature(dataURI), signed_date, sent_at }
  const [sentAt, setSentAt] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const applyNotice = (n: NoticeOfSaleData) => {
    const sa = (arr?: string[]) => (arr && arr.length) ? arr.join(', ') : '';
    if (buyersEditable) {
  ...
  // ---- signatures ----
  const setAgentDate = (name: string, v: string) => setAgents((a) => ({ ...a, [name]: { ...(a[name] || {}), signed_date: v } }));
  const onSig = (name: string, file: File | undefined) => {
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => setAgents((a) => ({ ...a, [name]: { ...(a[name] || {}), signature: reader.result as string, signed_date: a[name]?.signed_date || todayISO() } }));
    reader.readAsDataURL(file);
  };
  const clearSig = (name: string) => setAgents((a) => ({ ...a, [name]: { ...(a[name] || {}), signature: null } }));

  const toArr = (s: string) => s.split(',').map((x) => x.trim()).filter(Boolean);
  const persist = async (): Promise<NoticeOfSaleData> => {
  ...
  const save = async () => {
    setSaving(true);
    try { await persist(); onSaved?.(); toast('Notice of Sale saved', 'ok'); }
    catch { toast('Could not save Notice of Sale', 'bad'); }
    finally { setSaving(false); }
  };
  const doSend = async (names: string[]) => {
    ...
    try {
      ...
      applyNotice(res);
      // Refresh the parent so the transaction header reflects "Notice of Sale Sent".
      onSaved?.();
  ...
  const buildPrintClone = (): HTMLElement | null => {
  ...
  const buildDocBase64 = async (): Promise<string | null> => {
    const clone = buildPrintClone();
      ...
      <div className="modal lg" style={{ maxHeight: '92vh', overflowY: 'auto' }}>
        ...
        <div ref={ref}>
          ...
          <div style={card}>
            ...
            <div style={grid3}>
              <Field label={priceLabel} value={formatCurrency(txn.price)} />
              <Field label="Sold Date" value={txn.offer_date} />
              <Field label="Closing Date" value={txn.closing_date} />
            </div>

            <div style={{ marginBottom: 14 }}>
              <span style={label}>{buyersLabel}</span>
              {buyersEditable
                ? <input data-noprint-style style={boxInput} value={buyers} onChange={(e) => setBuyers(e.target.value)} placeholder={buyersPlaceholder} />
            ...
            <div style={{ marginBottom: 4 }}>
              <span style={label}>{sellersLabel}</span>
              {sellersEditable
                ? <input data-noprint-style style={boxInput} value={sellers} onChange={(e) => setSellers(e.target.value)} placeholder={sellersPlaceholder} />
