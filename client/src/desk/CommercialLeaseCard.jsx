...
const minusDay = (d) => { const x = new Date(d); x.setDate(x.getDate() - 1); return x; };

// Per-year escalated base rent (contractual), before any free-rent reduction.
function escalatedRent(base, rate, type, year) {
  const y = year - 1;
  switch (type) {
...
function tierForYear(method, year, primary, tiers) {
...
export default function CommercialLeaseCard({ cl, setCl, ro }) {
  const term = Math.max(1, Math.min(25, parseInt(cl.lease_term_years, 10) || 1));
  ...
  const field = (label, key, props = {}) => (
    <div className="field" style={{ marginBottom: 0 }}>
      ...
      <input value={cl[key] ?? ''} disabled={ro} onChange={(e) => setCl(key, e.target.value)} {...props} />
    </div>
  );
  const select = (label, key, options) => (
    <div className="field" style={{ marginBottom: 0 }}>
      <label style={lbl}>{label}</label>
      <select value={cl[key] ?? ''} disabled={ro} onChange={(e) => setCl(key, e.target.value)}>
