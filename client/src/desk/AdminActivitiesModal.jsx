...
export default function AdminActivitiesModal({ open, onClose, transactionId, txn, onSaved, termCount: termCountProp }) {
  ...
  const [form, setForm] = useState(() => {
    ...
    return {
      ...
      recv_lawyer: a.recv_lawyer || { enabled: '', via: '', date: '' },
      paid_lawyer: a.paid_lawyer || { enabled: '', via: '', date: '', batch: '' },
      paid_client: a.paid_client || { enabled: '', via: '', date: '', batch: '' },
  ...
  const rmRow = (n, key, i) => setForm((f) => ({ ...f, agents: { ...f.agents, [n]: { ...f.agents[n], [key]: f.agents[n][key].filter((_, idx) => idx !== i) } } }));
  ...
  const rmDeposit = (i) => setForm((f) => ({ ...f, deposits: f.deposits.filter((_, idx) => idx !== i) }));
  ...
  const rmTRow = (k, n, key, i) => setForm((f) => { const t = f.term_admin[k]; const ag = t.agents[n]; return { ...f, term_admin: { ...f.term_admin, [k]: { ...t, agents: { ...t.agents, [n]: { ...ag, [key]: ag[key].filter((_, idx) => idx !== i) } } } } }; });
  ...
  const save = async () => {
    ...
    try {
      ...
      toast('Admin saved', 'ok'); onSaved?.(updated); onClose();
      ...
      <div className="modal lg">
                    ...
                    <div className="pay-item" key={n} style={{ background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 10 }}>
                        ...
                        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                          ...
                          <select style={{ width: 'auto' }} value={ag.invoice_received} onChange={(e) => setTAgentField(k, n, { invoice_received: e.target.value })}><option>N/A</option><option>Yes</option><option>No</option></select>
                      ...
                      {ag.payments.map((p, i) => (
                        <div className="g4" key={i} style={{ alignItems: 'end', marginBottom: 6 }}>
                          <div className="field" style={{ marginBottom: 0 }}><label style={lbl}>Paid Type</label><select value={p.paid_type} onChange={(e) => setTRow(k, n, 'payments', i, { paid_type: e.target.value })}><option>N/A</option><option>TDB-EFT</option><option>CTA-BA Transfer</option><option>Cheque</option></select></div>
                    ...
                    <div className="xfer-item" key={n}>
                      ...
                      {ag.cta.map((c, i) => (
                        <div className="g3" key={i} style={{ alignItems: 'end', marginBottom: 6 }}>
                          <div className="field" style={{ marginBottom: 0 }}><label style={lbl}>CTA to BA</label><select value={c.cta} onChange={(e) => setTRow(k, n, 'cta', i, { cta: e.target.value })}><option>No</option><option>Yes</option><option>N/A</option></select></div>
        ...
        {!listing ? (<>
          ...
          <div className="g2" style={{ marginTop: 14 }}>
            <div className="field"><label style={lbl}>Client Void cheque Received</label><select value={form.void_cheque_received} onChange={(e) => set('void_cheque_received', e.target.value)}><option value="">Select</option><option>Yes</option><option>No</option><option>N/A</option></select></div>
          ...
          {!hideLawyer && (<>
            ...
            <div className="field" style={{ maxWidth: 240 }}><label style={lbl}>Received from Lawyer?</label><select value={form.recv_lawyer.enabled} onChange={(e) => setObj('recv_lawyer', { enabled: e.target.value })}><option value="">Select</option><option>Yes</option><option>No</option><option>N/A</option></select></div>
            ...
            <div className="field" style={{ maxWidth: 240 }}><label style={lbl}>Paid to Lawyer?</label><select value={form.paid_lawyer.enabled} onChange={(e) => setObj('paid_lawyer', { enabled: e.target.value })}><option value="">Select</option><option>Yes</option><option>No</option><option>N/A</option></select></div>
          ...
          <div className="field" style={{ maxWidth: 240 }}><label style={lbl}>Paid to Client?</label><select value={form.paid_client.enabled} onChange={(e) => setObj('paid_client', { enabled: e.target.value })}><option value="">Select</option><option>Yes</option><option>No</option><option>N/A</option></select></div>
          {form.paid_client.enabled === 'Yes' && (
            <div className="g3">
          ...
          <div className="field" style={{ maxWidth: 240 }}><label style={lbl}>Co-op invoice received?</label><select value={form.coop_invoice.enabled} onChange={(e) => setObj('coop_invoice', { enabled: e.target.value })}><option value="">Select</option><option>Yes</option><option>No</option><option>N/A</option></select></div>
          ...
          <div className="field" style={{ maxWidth: 240 }}><label style={lbl}>Transferred TA → CTA?</label><select value={form.ta_cta.enabled} onChange={(e) => setObj('ta_cta', { enabled: e.target.value })}><option value="">Select</option><option>Yes</option><option>No</option><option>N/A</option></select></div>
          ...
          return (
            <div className="pay-item" key={n} style={{ background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <label style={{ fontSize: 12, color: '#166534', margin: 0, fontWeight: 600 }}>Agent Invoice Received</label>
                  <select style={{ width: 'auto' }} value={a.invoice_received} onChange={(e) => setAgent(n, 'invoice_received', e.target.value)}><option>N/A</option><option>Yes</option><option>No</option></select>
              ...
              {a.payments.map((p, i) => (
                <div className="g4" key={i} style={{ alignItems: 'end', marginBottom: 6 }}>
                  <div className="field" style={{ marginBottom: 0 }}><label style={lbl}>Paid Type</label>
                    <select value={p.paid_type} onChange={(e) => setRow(n, 'payments', i, { paid_type: e.target.value })}><option>N/A</option><option>TDB-EFT</option><option>CTA-BA Transfer</option><option>Cheque</option></select></div>
              ...
              {a.cta.map((c, i) => (
                <div className="g3" key={i} style={{ alignItems: 'end', marginBottom: 6 }}>
                  <div className="field" style={{ marginBottom: 0 }}><label style={lbl}>CTA to BA</label>
                    <select value={c.cta} onChange={(e) => setRow(n, 'cta', i, { cta: e.target.value })}><option>No</option><option>Yes</option><option>N/A</option></select></div>
        ...
        {form.notes.map((note, i) => (
          <div className="note-item" key={i}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--muted)' }}><span>{note.date} — Admin</span><button className="row-rm" onClick={() => set('notes', form.notes.filter((_, idx) => idx !== i))}>🗑️</button></div>
          </div>
        ))}
