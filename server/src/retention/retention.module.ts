 ...
 *
 * IT IS SAFE TO SCHEDULE BEFORE IT IS SAFE TO RUN. Without `RETENTION_ENABLED=true` the sweep is a
  ...
  onModuleInit(): void {
    ...
    this.log.log(
      this.retention.enabled()
        ? 'Retention sweep scheduled — RETENTION_ENABLED=true, so it WILL delete Transaction Desk records past six months.'
        : 'Retention sweep scheduled in DRY RUN — it will report what six months would remove and delete nothing. Set RETENTION_ENABLED=true to act.',
    );
